...
SELECT cTitle, cName FROM library.tbook, library.ttheme, tbooktheme WHERE tbook.nBookID = tbooktheme.nBookID AND ttheme.nThemeID = tbooktheme.nThemeID AND tbook.nPublishingCompanyID = 32;
...
SELECT cName, cSurname, COUNT(tauthorship.nBookID IS NOT NULL ) AS bookPublished FROM tauthor JOIN tauthorship ON tauthor.nAuthorID = tauthorship.nAuthorID GROUP BY cName, cSurname;
...
SELECT cName name, cSurname surname, MIN(nPublishingYear) AS firstPublication FROM tauthor JOIN tauthorship t ON tauthor.nAuthorID = t.nAuthorID JOIN tbook t2 on t.nBookID = t2.nBookID GROUP BY cName, cSurname;
...
SELECT ttheme.cName, tbook.cTitle FROM ttheme LEFT JOIN tbooktheme On ttheme.nThemeID = tbooktheme.nThemeID LEFT JOIN tbook ON tbooktheme.nBookID = tbook.nBookID;
...
SELECT tmember.cName, tmember.cSurname, tmember.dNewMember, tbook.cTitle FROM tmember JOIN tloan ON tmember.cCPR = tloan.cCPR join tbookcopy ON tloan.cSignature = tbookcopy.cSignature JOIN tbook On tbookcopy.nBookID= tbook.nBookID WHERE dNewMember BETWEEN "2013-01-01" AND "2013-12-31" ORDER BY tmember.cName, tmember.cSurname;
...
SELECT tauthor.cName, tauthor.cSurname, tcountry.cName, tbook.cTitle FROM tauthor JOIN tnationality ON tauthor.nAuthorID = tnationality.nAuthorID JOIN tcountry ON tnationality.nCountryID = tcountry.nCountryID JOIN tauthorship ON tauthor.nAuthorID = tauthorship.nAuthorID JOIN tbook ON tauthorship.nBookID = tbook.nBookID ORDER BY tauthor.cName, tauthor.cSurname;

-- 17 Show the title of those books which have had different editions published in both 1970 and 1989.
SELECT tbook.cTitle, COUNT(*) AS c FROM tbook WHERE nPublishingYear = 1970 OR nPublishingYear = 1989 GROUP BY tbook.cTitle HAVING c > 1;
...
SELECT tmember.cName, tmember.cSurname FROM tmember WHERE dNewMember BETWEEN "2013-12-01" AND "2013-12-31" UNION SELECT tauthor.cName, tauthor.cSurname FROM tauthor WHERE cName = "William";
